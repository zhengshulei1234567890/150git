			...
			<p>
				...
				<select id="address" onchange="changeAddress(this.value)">
					...
					<c:forEach items="${list}" var="addr">
						<option value="${addr.id }" <c:if test="${addr.id == addressId}">selected</c:if> >${addr.local }</option>
					</c:forEach>					
				</select>
				<script type="text/javascript">
					function changeAddress(addressId){
						window.location.href="${pageContext.request.contextPath}/address/selectAddressesId?addressId="+addressId;
					}
				</script>
			</p>
			<form name="ctl00" method="post" action="${pageContext.request.contextPath }/address/addOrder" id="f">
				<input type="hidden" name="address.id" value="${address.id }" />
				<input type="hidden" name="address.user_id" value="${loginUser.id }" />
				...
				<div class="login_in">
					<a href="../front/order/order_info.jsp">
