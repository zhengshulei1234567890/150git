...
<body style="background-color: #f0f9fd;text-align: center">
	...
	<form action="${pageContext.request.contextPath}/book/updateBook" method="post" enctype="multipart/form-data">
			<table>
					...
					<td>
						<select name="book.categoryId" class="el-select__inner inner2">
							<c:forEach items="${seconds}" var="category">
								<option value="${category.id }" <c:if test="${book.categoryId == category.id }">selected</c:if>  >${category.name }</option>
							</c:forEach>
						</select>
					</td>
					<td>字数：</td>
				...
				<tr>
					<td>印刷时间：</td>
					<td><input type="date" name="book.printDate" class="el-input__inner" value="<fmt:formatDate value='${book.printDate }' pattern='yyyy-MM-dd' />"></td>
					<td>作者简介：</td>
